...
The user sends login details to the login API.

If the credentials are valid, a JWT token is generated.

The token is returned to the client.

For secured APIs, the client must send the token in the request header.

A JWT filter checks the token and allows access if it is valid.
